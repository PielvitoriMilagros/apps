...
@Component({
  selector: 'app-sala-cuarto-a',
  templateUrl: './sala-cuarto-a.component.html',
  styleUrls: ['./sala-cuarto-a.component.scss'],
})
export class SalaCuartoAComponent implements OnInit {
    ...
    private userServ:UsuariosService) { 
      ...
      auth.currentUser().then(resp=>{
        if(resp!=null){
          ...
          userServ.getMensajesA().subscribe((res:any)=>{
            let msj;
            let array=new Array();
...
enviarMensaje(){
  this.userServ.saveMsj(this.msjNuevo,this.ucorreo,'mensajesA').then(resp =>{
    this.msjNuevo=null;
  });
}
